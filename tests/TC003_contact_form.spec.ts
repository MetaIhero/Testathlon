...
test.describe("TC003 – Kapcsolatfelvételi Űrlap (Contact Form) | KÖZEPES prioritás", () => {
  ...
  test("TC003-01 – Sikeres üzenetküldés érvényes adatokkal", async ({
    page,
  }) => {
    ...
    await page.evaluate(() => {
      const el = document.querySelector(
        'form, section#contact, [id*="contact"]',
      );
    ...
    const nameInput = page
      .locator('label:has-text("Name") ~ input, input[name="name"]')
      .first();
    const emailInput = page
      .locator('label:has-text("Email") ~ input, input[name="email"]')
      .first();
    const phoneInput = page
      .locator('label:has-text("Phone") ~ input, input[name="phone"]')
      .first();
    const subjectInput = page
      .locator('label:has-text("Subject") ~ input, input[name="subject"]')
      .first();
    ...
    await messageInput.fill(
      "Érdeklődöm 2026 júniusában elérhető szobák iránt. Kérjük, vegyék fel velem a kapcsolatot.",
    );
  ...
  test("TC003-02 – Validáció hiányos adatokkal (negatív eset)", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
