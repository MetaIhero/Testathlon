...
export class RoomDetailPage {
  ...
  constructor(page: Page) {
    ...
    this.totalPrice = page.locator('text=/Total/').locator('..').locator('text=/£\d+/');
  ...
  async getTotalPriceText(): Promise<string> {
    ...
    return rows.first().locator('xpath=..').textContent() ?? '';
  }

  async getCalendarMonth(): Promise<string> {
    return (await this.page.locator('text=/January|February|March|April|May|June|July|August|September|October|November|December/').first().textContent()) ?? '';
  }
}
