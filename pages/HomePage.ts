...
export class HomePage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.checkInInput = page.locator('input[name="checkin"], input[placeholder*="Check"], .checkin input').first();
    this.checkOutInput = page.locator('input[name="checkout"], input[placeholder*="Check"], .checkout input').first();
    this.checkAvailabilityButton = page.getByRole('button', { name: 'Check Availability' });
    this.bookNowHeroButton = page.getByRole('link', { name: 'Book Now' }).first();
